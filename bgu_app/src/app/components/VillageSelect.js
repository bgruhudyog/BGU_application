...
export default function VillageSelect({ villages, setShops, routeId, setVillageInfo }) {
  const [newVillageName, setNewVillageName] = useState('');
  const [addingVillage, setAddingVillage] = useState(false);
  ...
  const addVillage = async () => {
    ...
    if (error) console.error('Error adding village:', error);
      ...
      const { data: updatedVillages } = await supabaseClient.from('Villages Table').select('*').eq('route_id', routeId);
